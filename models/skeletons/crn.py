...
class CRN(nn.Module): # hj
    ...
    def __init__(self, cfg):
        ...
        # For radar + camera only fusion (no LiDAR in backbone_2d)
        # map_to_bev_module.num_bev_features = [lidar_channels, radar_channels] = [64, 64]
        # We only use radar (index 1) + camera for the cross-attention fusion
        radar_bev_features = self.map_to_bev_module.num_bev_features[1]  # 64 for radar
        camera_bev_features = model_info_dict['num_bev_features_cam']    # 64 for camera
        model_info_dict['num_bev_features'] = np.array(self.map_to_bev_module.num_bev_features).sum()  # keep for compatibility
        self.backbone_2d = backbone_2d.__all__[self.model_cfg.BACKBONE_2D.NAME](
            model_cfg=self.model_cfg.BACKBONE_2D,
            input_channels=radar_bev_features + camera_bev_features  # 128 (radar + camera only)
        )
