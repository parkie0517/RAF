# * modified from: pp_rlf.py
# * usage: for training voxel l4dr
# * created: hj park
    ...
    def pre_processor(self, batch_dict):
        if self.is_pre_processing is None:
            ...
            for batch_idx in range(batch_dict['batch_size']):
                temp_points = batched_rdr[torch.where(batched_indices_rdr == batch_idx)[0],:self.num_point_features[1]] # batch가 batch_idx인 점들 추출
                
                if (self.shuffle_points) and (self.training):
                    shuffle_idx = np.random.permutation(temp_points.shape[0]) # 셔플로 인해서 temp_points 순서 뒤바뀜
