"""
* skeleton code for training simple sprase voxel encoder
* editted by HJ
* copied from pp_crlf_uem.py
"""
...
class Voxel_L(nn.Module): # hj
    """
    Simple sparse voxel encoder base skeleton
    """
    def __init__(self, cfg):
        ...
        voxel_size = self.dataset_cfg.roi.voxel_size # [0.4, 0.4, 0.4]
        ...
        # # self.fuser = build_fuser(self.model_cfg.FUSER)
        
        ###                           ###
        ### ----- Camera Branch ----- ###
        #################################
        
        # 3D sparse voxel feature -> BEV Feature map # NOTE 필요 없으면 삭제 ㄱㄱ
        # self.voxel_to_bev = backbone_2d.__all__[self.model_cfg.VOXEL_TO_BEV.NAME](
        #     model_cfg=self.model_cfg.VOXEL_TO_BEV,
        #     point_cloud_range=model_info_dict['point_cloud_range'], # [ 0. , -6.4, -2. , 72. ,  6.4,  6. ]
        #     grid_size=self.model_cfg.UEM_LDR_PREPROCESSOR.GRID_SIZE, # 0.4
        #     encoding_stride=self.model_cfg.UEM_BACKBONE_3D.ENCODING.STRIDE, # [1, 2, 2]
        # )
        ...
        # BEV Feature map encoder
        # model_info_dict['num_bev_features'] = np.array(self.map_to_bev_module.num_bev_features).sum()
        # model_info_dict['num_bev_features'] = self.model_cfg.UEM_BACKBONE_3D.ENCODING.CHANNEL[-1] # 256
        ...
        # bev 인코더를 사용하지 않고 바로 detection head에 bev feature를 넣을 거임
        model_info_dict['num_bev_features'] = self.uem_encoder.bev_output_channel # 768
        self.dense_head = head.__all__[self.model_cfg.DENSE_HEAD.NAME](
            model_cfg=self.model_cfg.DENSE_HEAD,
            input_channels=model_info_dict['num_bev_features'] if 'num_bev_features' in model_info_dict else self.model_cfg.DENSE_HEAD.INPUT_FEATURES, # 768
            num_class=self.num_class if not self.model_cfg.DENSE_HEAD.CLASS_AGNOSTIC else 1, # 1
            class_names=self.class_names, # ['Sedan']
            grid_size=model_info_dict['grid_size'], # simple voxel encoder에서는 [180, 32, 1], l4dr학습할 때는 [480, 80, 1]
            point_cloud_range=model_info_dict['point_cloud_range'],
            predict_boxes_when_training=self.model_cfg.get('ROI_HEAD', False),
            voxel_size=model_info_dict.get('voxel_size', False) # simple voxel encoder에서는 [0.4, 0.4, 0.4] l4dr에서는 [0.15, 0.16, 8]
        )
        ...
        # Pre-processor
        self.is_pre_processing = self.model_cfg.PRE_PROCESSING.get('VER', None) # 'v1_0'
        self.shuffle_points = self.model_cfg.PRE_PROCESSING.get('SHUFFLE_POINTS', False) # True
        self.transform_points_to_voxels = self.model_cfg.PRE_PROCESSING.get('TRANSFORM_POINTS_TO_VOXELS', False) # True
    ...
    def forward(self, batch_dict):
        ...
        batch_dict = self.pre_processor(batch_dict) # gt boxes cuda로 올려줘야함
        ...
        # batch_dict = self.uem(images, batch_dict)
        # conf_map = batch_dict['conf_map'].unsqueeze(1) # [B, 1, H, W]
        # images = images*conf_map
        
        
        # Sparse Conv data -> 2D BEV Feature Map
        # batch_dict = self.voxel_to_bev(batch_dict)

        
        # BN, C, H, W = images.size()
        # images = images.view(B, int(BN / B), C, H, W) # [B, 256, 80, 160] -> [B, 1, 256, 80, 160]
        ...
        #     None, # img_metas, 
        #     depth_loss = None, # self.use_depth_loss, 
        #     gt_depths = None, #gt_depths, 
        # )
        # batch_dict['camera_spatial_features'] = cam_bev_feat.permute(0, 1, 3, 2) # [4, 64, 240, 40] -> [4, 64, 40, 240]
        # self.fuser
        ### --------------- Camera Branch --------------- ###
        #####################################################
