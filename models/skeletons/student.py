...
class Student(nn.Module): # hj
    ...
    def __init__(self, cfg):
        ...
        self.point_head = None # cam only 때는 None으로 해야 함.
    ...
    def forward(self, batch_dict):
        ...
        cam_bev_out = self.vtransform(
            ...
            depth_gt = batch_dict.get('depth_gt', None),
        )
        if isinstance(cam_bev_out, (tuple, list)):
            cam_bev_feat = cam_bev_out[0]
            if len(cam_bev_out) > 1:
                batch_dict['depth_pred'] = cam_bev_out[1]
            if len(cam_bev_out) > 2:
                batch_dict['depth_gt'] = cam_bev_out[2]
        else:
            cam_bev_feat = cam_bev_out

        batch_dict['camera_spatial_features'] = cam_bev_feat.permute(0, 1, 3, 2) # [B, 64, 100, 240]
        

        
        batch_dict = self.dense_head(batch_dict) # head
    ...
    def loss(self, dict_item):
        ...
        depth_loss = 0
        if hasattr(self.vtransform, 'loss_cfg'):
            loss_cfg = self.vtransform.loss_cfg
            if loss_cfg.get('DEPTH_SUPERVISION', False):
                if ('depth_pred' in dict_item) and ('depth_gt' in dict_item):
                    depth_pred = dict_item['depth_pred']
                    depth_gt = dict_item['depth_gt']
                    # depth_pred/gt: [B, N, D, H, W]
                    B, N, D, H, W = depth_pred.shape
                    depth_pred = depth_pred.permute(0, 1, 3, 4, 2).contiguous().view(-1, D)
                    depth_gt = depth_gt.permute(0, 1, 3, 4, 2).contiguous().view(-1, D)
                    fg_mask = depth_gt.sum(dim=1) > 0
                    if fg_mask.any():
                        ...
                        depth_loss = depth_loss * loss_cfg.get('DEPTH_SUPERVISION_WEIGHT', 1.0)
