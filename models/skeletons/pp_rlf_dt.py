...
class PointPillar_RLF_DT(nn.Module):
    def __init__(self, cfg, mode='train'):
        ...
        # Test mode: Skip (full checkpoint loaded by main_eval)
    ...
    def _freeze_weights(self):
        ...
        Stage 3: Freeze all (teacher for KD)
        """
        if self.stage == 1:
        ...
        if self.stage == 2:
            ...
            # Stage 3: Freeze all (teacher model for KD)
            for param in self.parameters():
                param.requires_grad = False

            total_params = sum(p.numel() for p in self.parameters())
            print(f"[Stage 3] All {total_params:,} params frozen (teacher mode for KD)")

    def _load_pretrained_weights(self):
        """
        Stage 1: No pretrained weights needed
        Stage 2: Load L4DR pretrained weights (backbone_2d_rt is randomly initialized)
        Stage 3: Will be implemented later (load all weights)
        """
        if self.stage == 1:
            return
