...
# editted by HJ
# Added cnn based UEM to PointPillar_CRLF_UEM
...
from models import backbone_2d, backbone_3d, head, roi_head, fusers, pre_processor
...
class CRN(nn.Module): # hj
    """
    fusion of crl in bev space + UEM_cnn
    """
    def __init__(self, cfg):
        ...
        # camera branch
        self.img_backbone = build_backbone(self.model_cfg.IMG_BACKBONE)
        self.img_neck = build_neck(self.model_cfg.IMG_NECK)

        self.vtransform = build_vtransform(self.model_cfg.VTRANSFORM)
        model_info_dict['num_bev_features_cam'] = self.model_cfg.VTRANSFORM.out_channels
      
        # --- Radar Modules ---
        self.vfe = vfe.__all__[self.model_cfg.VFE.NAME](
            model_cfg=self.model_cfg.VFE,
            num_point_features=[num_point_features[1]], # Radar Only
        ...
        self.backbone_3d = backbone_3d.__all__[self.model_cfg.BACKBONE_3D.NAME](
            ...
            input_channels=self.vfe.get_output_feature_dim(), # From Radar VFE
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range']
        )
        ...
        # --- LiDAR Modules (New) ---
        self.lidar_vfe = vfe.__all__[self.model_cfg.VFE.NAME](
            model_cfg=self.model_cfg.VFE,
            num_point_features=[num_point_features[0]], # LiDAR Only
            point_cloud_range=model_info_dict['point_cloud_range'],
            voxel_size=model_info_dict['voxel_size'],
            grid_size=model_info_dict['grid_size'],
        )
        
        self.lidar_backbone_3d = backbone_3d.__all__[self.model_cfg.BACKBONE_3D.NAME](
            model_cfg=self.model_cfg.BACKBONE_3D,
            input_channels=self.lidar_vfe.get_output_feature_dim(), # From LiDAR VFE
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range']
        )

        self.lidar_map_to_bev_module = map_to_bev.__all__[self.model_cfg.MAP_TO_BEV.NAME](
            model_cfg=self.model_cfg.MAP_TO_BEV,
            grid_size=model_info_dict['grid_size']
        )
        ...
        self.point_head = head.__all__[self.model_cfg.POINT_HEAD.NAME](
            ...
            input_channels=self.backbone_3d.backbone_channels if hasattr(self.backbone_3d, 'backbone_channels') else self.vfe.get_output_feature_dim(),
            num_class=self.num_class if not self.model_cfg.POINT_HEAD.CLASS_AGNOSTIC else 1,
            predict_boxes_when_training=self.model_cfg.get('ROI_HEAD', False)
        )

        # --- Fusion & Detection ---
        radar_bev_features = self.map_to_bev_module.num_bev_features[1] if isinstance(self.map_to_bev_module.num_bev_features, list) else self.map_to_bev_module.num_bev_features
        lidar_bev_features = self.lidar_map_to_bev_module.num_bev_features[1] if isinstance(self.lidar_map_to_bev_module.num_bev_features, list) else self.lidar_map_to_bev_module.num_bev_features
        # Depending on map_to_bev implementation, num_bev_features might be int or list. Assume int or pick last?
        # Usually it's int. If list (like in config comments), picking the relevant one.
        # Assuming int here based on standard OpenPCDet, but if list, [0]=lidar, [1]=radar?
        # For safety, let's grab .num_bev_features directly if it's an int.
        
        if isinstance(self.map_to_bev_module.num_bev_features, list):
             radar_bev_features = self.map_to_bev_module.num_bev_features[1] # Assuming standard OpenPCDet multi-modal config structure if list
        else:
             radar_bev_features = self.map_to_bev_module.num_bev_features

        if isinstance(self.lidar_map_to_bev_module.num_bev_features, list):
             lidar_bev_features = self.lidar_map_to_bev_module.num_bev_features[0] # Just a guess, using 0 for lidar? Or same logic.
        else:
             lidar_bev_features = self.lidar_map_to_bev_module.num_bev_features

        camera_bev_features = model_info_dict['num_bev_features_cam']
        
        # Total channels = Radar + LiDAR + Camera
        total_bev_features = radar_bev_features + lidar_bev_features + camera_bev_features
        
        self.backbone_2d = backbone_2d.__all__[self.model_cfg.BACKBONE_2D.NAME](
            model_cfg=self.model_cfg.BACKBONE_2D,
            input_channels=total_bev_features
        )
    ...
    def pre_processor(self, batch_dict):
        if self.is_pre_processing is None:
            ...
            ### --------------- Camera Branch --------------- ###        
            ### ----- LSS ----- ###
            batch_dict['rdr_points_list'] = [p.cuda(non_blocking=True) for p in list_points] # lss 모듈에서 depth만들 때 활용됨
            ### ----- LSS ----- ###
            ### --------------- Camera Branch --------------- ###
            ...
            # LSS (also update lidar points list here for completeness, though pre_processor_cam does it too)
            batch_dict['ldr_points_list'] = [p.cuda(non_blocking=True) for p in list_points]

            return batch_dict
        
        
    def pre_processor_cam(self, batch_dict):
        ### --------------- Camera Branch --------------- ###
        if 'front0' in batch_dict:
            # camera images
            batch_dict['front0'] = batch_dict['front0'].unsqueeze(1).cuda()
            
            # camera parameters
            batch_dict['camera2lidar'] = batch_dict['camera2lidar'].cuda()
            batch_dict['lidar2camera'] = batch_dict['lidar2camera'].cuda()
            batch_dict['lidar2image'] = batch_dict['lidar2image'].cuda()
            batch_dict['camera2image'] = batch_dict['camera2image'].cuda()
            batch_dict['lidar2ego'] = batch_dict['lidar2ego'].cuda()
            batch_dict['image_aug_matrix'] = batch_dict['image_aug_matrix'].cuda()
            batch_dict['lidar_aug_matrix'] = batch_dict['lidar_aug_matrix'].cuda()
            
            # LSS
            batch_dict['ldr_points_list'] = [p.cuda() for p in batch_dict['ldr_points_list']]
        ### --------------- Camera Branch --------------- ###
    ...
    def forward(self, batch_dict):
        ...
        # --- 1. Foreground-Aware Denoising (FAD) for Radar ---
        # Prepare Radar Input for FAD
        # Note: We must ensure batch_dict has the correct 'voxels' for the radar branch.
        # pre_processor populates 'radar_voxels' and 'lidar_voxels'.
        # We need to map 'radar_voxels' to 'voxels' for the OpenPCDet modules to work.
        
        batch_dict['voxels'] = batch_dict['radar_voxels']
        batch_dict['voxel_coords'] = batch_dict['radar_voxel_coords']
        batch_dict['voxel_num_points'] = batch_dict['radar_voxel_num_points']
        
        # Apply VFE first! (Correcting logic from original crn.py)
        batch_dict = self.vfe(batch_dict) 
        batch_dict = self.backbone_3d(batch_dict) # radar feature extraction
        
        # Point Head for filtering
        batch_dict = self.point_head(batch_dict) # foreground segmentation
        ...
        # Filter raw radar points
        # Note: We are modifying 'rdr_sparse' based on the mask derived from 'point_cls_scores'
        # The 'point_cls_scores' correspond to the voxel features, but how do we map back to points?
        # The original code did this matching. Assuming point_head returns scores per point (PointPillar Scatter)?
        # If backbone_3d is PointPillars, it outputs (B, C, H, W). point_head outputs (B, Cls, H, W).
        # This masking logic seems to assume point-wise scores. 
        # For now, keeping the original logic structure but assuming it works for the specific config.
        # IF the original code was: backbone_3d -> point_head -> mask, it implies point_head produces point-wise scores.
        ...
        # RE-Generate Voxels for Radar (Filtered)
        batch_dict = self.pre_processor(batch_dict)
        
        # --- 2. Main Radar Branch (Filtered) ---
        batch_dict['voxels'] = batch_dict['radar_voxels']
        batch_dict['voxel_coords'] = batch_dict['radar_voxel_coords']
        batch_dict['voxel_num_points'] = batch_dict['radar_voxel_num_points']
        ...
        # Store Radar BEV features
        batch_dict['radar_spatial_features'] = batch_dict['spatial_features']
        ...
        batch_dict = self.lidar_backbone_3d(batch_dict)
        batch_dict = self.lidar_map_to_bev_module(batch_dict)
        
        # Store LiDAR BEV features
        batch_dict['lidar_spatial_features'] = batch_dict['spatial_features']

        # --- 4. Camera Branch ---
        batch_dict = self.pre_processor_cam(batch_dict)
        images = batch_dict['front0']
        B, N, C, H, W = images.size()
        images = images.view(B * N, C, H, W)

        images = self.img_backbone(images) 
        images = self.img_neck(images)
        
        if not isinstance(images, torch.Tensor):
            images = images[0] 
        
        batch_dict['feature_map'] =  images   

        BN, C, H, W = images.size()
        images = images.view(B, int(BN / B), C, H, W) 
        cam_bev_feat = self.vtransform(
            images, 
            batch_dict['ldr_points_list'], # Use LiDAR for depth supervision if available
            batch_dict['rdr_points_list'], 
            batch_dict['camera2lidar'], 
            batch_dict['lidar2ego'], 
            batch_dict['lidar2camera'], 
            batch_dict['lidar2image'], 
            batch_dict['camera2image'], 
            batch_dict['camera2lidar'], 
            batch_dict['image_aug_matrix'], 
            batch_dict['lidar_aug_matrix'], 
            None, 
            depth_loss = None, 
            gt_depths = None, 
        )
        batch_dict['camera_spatial_features'] = cam_bev_feat.permute(0, 1, 3, 2)

        # --- 5. Fusion & Detection ---
        # Concatenate: LiDAR + Radar + Camera
        batch_dict['spatial_features'] = torch.cat([
            batch_dict['lidar_spatial_features'],
            batch_dict['radar_spatial_features'],
            batch_dict['camera_spatial_features']
        ], dim=1)
    ...
    def loss(self, dict_item):
        loss_rpn, tb_dict = self.dense_head.get_loss() # detection loss
        if self.point_head is not None:
            # point_head loss logic (if it supports returning loss)
            # Ensure point_head has processed the correct data if it calculates loss during forward
            # Or if it stores targets. 
            # In OpenPCDet, get_loss often uses stored targets in self or batch_dict.
            # We must be careful if point_head expects data from the FIRST pass.
            loss_point, tb_dict_point = self.point_head.get_loss(tb_dict) 
