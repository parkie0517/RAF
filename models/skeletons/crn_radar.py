...
# Aligned with Official CRN (Camera Radar Net) Architecture
# Input: Camera + Radar
...
class CRN(nn.Module): 
    """
    CRN: Camera Radar Net (Camera + Radar only)
    """
    def __init__(self, cfg):
        ...
        # Radar Branch Modules
        self.vfe = vfe.__all__[self.model_cfg.VFE.NAME](
            model_cfg=self.model_cfg.VFE,
            num_point_features=[num_point_features[1]], # Explicitly Radar
        ...
        self.backbone_3d = backbone_3d.__all__[self.model_cfg.BACKBONE_3D.NAME](
            ...
            input_channels=model_info_dict['num_point_features'], # From VFE
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range']
        )
        
        # No point_head (FAD removed)
        ...
        if isinstance(self.map_to_bev_module.num_bev_features, list):
             radar_bev_features = self.map_to_bev_module.num_bev_features[1] 
        else:
        ...
        self.backbone_2d = backbone_2d.__all__[self.model_cfg.BACKBONE_2D.NAME](
            ...
            input_channels=radar_bev_features + camera_bev_features 
        )
    ...
    def pre_processor_cam(self, batch_dict):
        ...
        if 'front0' in batch_dict:
            ...
            # LSS
            # batch_dict['ldr_points_list'] ignored/removed
            if 'ldr_points_list' in batch_dict:
                 batch_dict.pop('ldr_points_list') # Ensure it's not used
        ### --------------- Camera Branch --------------- ###
    ...
    def forward(self, batch_dict):
        ...
        # --- Radar Branch (No FAD) ---
        # Map radar voxels to 'voxels' for OpenPCDet modules
