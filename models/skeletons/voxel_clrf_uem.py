...
# editted by HJ
# Skeleton code for CLR fusion + UEM (uncertainty estimation module)
...
from models import backbone_2d, backbone_3d, head, roi_head, fusers
...
class Voxel_CLRF_UEM(nn.Module):
    """
    fusion of clr and UEM (uncertainty estimation module)
    """
    def __init__(self, cfg):
        ...
        # self.vtransform = build_vtransform(self.model_cfg.VTRANSFORM)                         # NOTE uem학습때는 필요 X
        # model_info_dict['num_bev_features_cam'] = self.model_cfg.VTRANSFORM.out_channels      # NOTE uem학습때는 필요 X
        ...
        # 아래 보이는 건 Bidirectional Fusion 모듈
        ...
        # 아래 보이는 건 radar filtering용 pointnet2
        model_info_dict['num_point_features'] = self.vfe.get_output_feature_dim()
        self.backbone_3d_1 = backbone_3d.__all__[self.model_cfg.BACKBONE_3D_1.NAME](
            model_cfg=self.model_cfg.BACKBONE_3D_1,
        ...
        # 아래 보이는 건 radar filtering용 segmentation head
        model_info_dict['num_point_features'] = self.backbone_3d_1.num_point_features
        model_info_dict['backbone_channels'] = self.backbone_3d_1.backbone_channels \
            if hasattr(self.backbone_3d_1, 'backbone_channels') else None
        ...
        ####################################################################################
        ### --------------- 새로 추가하는 vfe와 map2bev 사이의 spconv unet --------------- ###
        ###                                                                              ###
        
        # 아래 보이는 건 LiDAR Sparse U-Net
        self.backbone_3d_2 = backbone_3d.__all__[self.model_cfg.BACKBONE_3D_2.NAME](
            model_cfg=self.model_cfg.BACKBONE_3D_2, 
            input_channels=self.vfe.get_output_feature_dim(), # 64
            grid_size=model_info_dict['grid_size'], # [480,  80,  50]
            voxel_size=model_info_dict['voxel_size'], # [0.15, 0.16, 0.16]
            point_cloud_range=model_info_dict['point_cloud_range'], # [ 0. , -6.4, -2. , 72. ,  6.4,  6. ]
        )
        
        # 아래 보이는 건 Radar Sparse U-Net
        # self.backbone_3d_3 = backbone_3d.__all__[self.model_cfg.BACKBONE_3D_3.NAME](
        #     model_cfg=self.model_cfg.BACKBONE_3D_3, 
        #     input_channels=self.vfe.get_output_feature_dim(), # 64
        #     grid_size=model_info_dict['grid_size'], # [480,  80,  50]
        #     voxel_size=model_info_dict['voxel_size'], # [0.15, 0.16, 0.16]
        #     point_cloud_range=model_info_dict['point_cloud_range'], # [ 0. , -6.4, -2. , 72. ,  6.4,  6. ]
        ...
        # 아래 보이는 건 3d tensor -> BEV feature map 변환 모듈. 일단 uem학습할 때는 필요 X
        # self.map_to_bev_module = map_to_bev.__all__[self.model_cfg.MAP_TO_BEV.NAME]( # voxel scatter
    ...
    def pre_processor(self, batch_dict):
        if self.is_pre_processing is None:
            ...
            list_voxel_num_points = []
            for batch_idx in range(batch_dict['batch_size']):
                temp_points = batched_ldr64[torch.where(batched_indices_ldr64 == batch_idx)[0],:self.num_point_features[0]] # 9개 특성 중 4개만 사용
                if (self.shuffle_points) and (self.training):
                    shuffle_idx = np.random.permutation(temp_points.shape[0])
                    temp_points = temp_points[shuffle_idx,:]
                list_points.append(temp_points)
    ...
    def forward(self, batch_dict):
        ...
        batch_dict = self.backbone_3d_1(batch_dict)
        batch_dict = self.point_head(batch_dict)
        pre_mask = batch_dict['point_cls_scores'] > self.model_cfg.PRE_PROCESSING.DENOISE_T # 0.3
        ...
        batch_dict['raw_rdr_sparse'] = batch_dict['rdr_sparse'][pre_mask] # NOTE 문제 가능!! 지금 rdr_sparse와 pre_mask의 idx가 매칭이 안 된다. 그래서 shuffle = False로 문제 해결함
        ...
        batch_dict = self.pre_processor(batch_dict) # 다시 preprocess하는 이유는, rdr_sparse가 새롭게 정의됐기 때문임.
        ...
        images = images.view(B, int(BN / B), C, H, W) # [4, 256, 80, 160] -> [4, 1, 256, 80, 160]
        
        # images = self.vtransform(
        #     images, # good
        ...
        # )
        # batch_dict['camera_spatial_features'] = images.permute(0, 1, 3, 2) # [4, 64, 240, 40] -> [4, 64, 40, 240] # NOTE (B, C, H, W)
        ...
        batch_dict = self.vfe(batch_dict) # BiDF VFE
        batch_dict = self.backbone_3d_2(batch_dict) # spconv unet LiDAR
        
        #############################################################
        ###                           UEM                         ###
        ###                                                       ###
        batch_dict['camera_feature_map'] = images # [1, 1, 256, 40, 80] # B, N, C, H, W
        ...
        # batch_dict = self.backbone_3d_3(batch_dict) # spconv unet radar
    ...
    def loss(self, dict_item):
        # loss_rpn, tb_dict = self.dense_head.get_loss() # NOTE uem만 학습할 때는 필요 없음
        if self.point_head is not None:
            tb_dict = None  # up up line에 주석처리해서 tb_dict가 정의 안됨. 그런 상태로 down라인 실행하면 애러 발생함. 그래서 tb_dict를 None을 초기화
            loss_point, tb_dict = self.point_head.get_loss(tb_dict)
        else:
            loss_point = 0
        # loss = loss_rpn + loss_point  # NOTE 이건 3d det loss 줄 때 쓰는 거
        loss =  loss_point              # NOTE 지금은 pointnet seg loss랑 유사도 loss만 주기
