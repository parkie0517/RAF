...
class UEM_CNN_CONTRASTIVE_MAX_WINDOW(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # 4. Sample image features in local window and keep max-similarity location
        valid_b = b[valid_mask] # [N_new]
        x_center = proj_coords[:, 0].long()  # [N_new]
        y_center = proj_coords[:, 1].long()  # [N_new]
        ...
        best_sim = torch.full((proj_lidar_feat.shape[0],), float('-inf'), device=proj_lidar_feat.device)
        best_x = x_center.clone()
        best_y = y_center.clone()
        has_candidate = torch.zeros_like(best_sim, dtype=torch.bool)

        for dy in range(-half_w, half_w + 1):
            for dx in range(-half_w, half_w + 1):
                cand_x = x_center + dx
                cand_y = y_center + dy
                in_bounds = (cand_x >= 0) & (cand_x < feat_w) & (cand_y >= 0) & (cand_y < feat_h)

                if not torch.any(in_bounds):
                    continue

                sampled_feat = feature_map[valid_b[in_bounds], :, cand_y[in_bounds], cand_x[in_bounds]] # [N_valid, 256]
                proj_image_feat = self.image_projector(sampled_feat) # [N_valid, 256] -> [N_valid, 128]
                cand_sim = F.cosine_similarity(proj_lidar_feat[in_bounds], proj_image_feat, dim=-1) # [N_valid]

                prev_sim = best_sim[in_bounds]
                update_mask = cand_sim > prev_sim

                update_indices = torch.nonzero(in_bounds, as_tuple=False).squeeze(1)[update_mask]
                best_sim[update_indices] = cand_sim[update_mask]
                best_x[update_indices] = cand_x[in_bounds][update_mask]
                best_y[update_indices] = cand_y[in_bounds][update_mask]
                has_candidate[in_bounds] = True

        self.valid_b = valid_b[has_candidate]
        self.x = best_x[has_candidate]
        self.y = best_y[has_candidate]
        self.similarity = best_sim[has_candidate] # loss 계산할 때 사용
