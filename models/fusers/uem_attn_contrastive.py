...
class ConfHead(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        # 1x1 projection keeps spatial resolution while preparing token embeddings
        ...
        # lightweight positional encoding built from normalized 2D coordinates
    ...
    def forward(self, feature_map):
        ...
        x = self.input_proj(feature_map)                 # [B, hidden_dim, H, W]
        x = x.flatten(2).transpose(1, 2)                 # [B, H*W, hidden_dim]

        # build sinusoid-free positional tokens from normalized 2D coords
        ys = torch.linspace(-1.0, 1.0, steps=H, device=device)
        xs = torch.linspace(-1.0, 1.0, steps=W, device=device)
        grid_y, grid_x = torch.meshgrid(ys, xs, indexing='ij')
        pos = torch.stack((grid_x, grid_y), dim=-1).view(1, H * W, 2)
        pos = self.pos_mlp(pos).expand(B, -1, -1)        # [B, H*W, hidden_dim]

        x = x + pos
        x = self.transformer(x)                          # [B, H*W, hidden_dim]
        conf = self.output_head(x)                       # [B, H*W, 1]
        conf = conf.transpose(1, 2).view(B, 1, H, W)     # [B, 1, H, W]
        return conf
    

class UEM_attn_contrastive(nn.Module):
    def __init__(self,
                 model_cfg,
