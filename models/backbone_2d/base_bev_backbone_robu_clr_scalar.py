...
class BaseBEVBackbone_Robu_CLR_scalar(nn.Module):
    """
    ...
    def __init__(self, model_cfg, input_channels):
        ...
        # Degradation-aware head for camera features (confidence scalar)
    ...
    def forward(self, data_dict):
        ...
        # Compute confidence scalar and weight camera features
        camera_conf = self.degradation_aware_head(camera_x)  # [B, 1, H, W]
        camera_conf = camera_conf.mean(dim=(2, 3))  # [B, 1]
        camera_conf = torch.sigmoid(camera_conf)  # [B, 1] in [0, 1]
        camera_conf_broadcast = camera_conf.view(-1, 1, 1, 1)  # [B, 1, 1, 1]
        camera_x = camera_x * camera_conf_broadcast  # [B, 64, 80, 480]
        # radar_x = radar_x * (1.0 - camera_conf_broadcast)  # [B, 64, 80, 480]
