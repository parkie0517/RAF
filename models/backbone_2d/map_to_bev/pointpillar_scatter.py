...
class PointPillarScatter(nn.Module):
    def __init__(self, model_cfg, grid_size, **kwargs):
        ...
        self.num_bev_features = self.model_cfg.NUM_BEV_FEATURES # [64, 64]
        self.nx, self.ny, self.nz = grid_size # (480, 80, 50)
        assert self.nz == 1

    def forward(self, batch_dict, **kwargs):
        if 'pillar_features' in batch_dict: # pass
        ...
        elif 'common_pillar_features' in batch_dict: # pass
            ...
            lidar_pillar_features, lidar_coords = batch_dict['lidar_pillar_features'], batch_dict['lidar_voxel_coords'] # [4575, 64], [4575, 4], lidar_coords[:, 1].unique() -> 0
            radar_pillar_features, radar_coords = batch_dict['radar_pillar_features'], batch_dict['radar_voxel_coords'] # [3688, 64], [3688, 4]
            lidar_batch_spatial_features = []
            radar_batch_spatial_features = []
            ...
            for lidar_batch_idx in range(batch_size): # batch_size = 2
                lidar_spatial_feature = torch.zeros(
                    self.num_bev_features[0], # 64
                    self.nz * self.nx * self.ny, # 1, 480, 80
                    dtype=lidar_pillar_features.dtype,
                    device=lidar_pillar_features.device)
            ...
            batch_dict['lidar_spatial_features'] = lidar_batch_spatial_features
            batch_dict['radar_spatial_features'] = radar_batch_spatial_features
            if 'de_lidar_pillar_features' in batch_dict.keys(): # pass
