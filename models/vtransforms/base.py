...
class BaseTransform(nn.Module):
    ...
    def get_geometry(
        ...
        post_trans,
        **kwargs,
    ): # 2D 이미지의 depth를 알 때, 3d 공간에 어디에 위치하는지 알려주는 mapping 정보를 생성하는 함수.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # x = self.get_cam_feats(img, mats_dict) # mets_dict있으면 애러 남
...
class BaseDepthTransform(BaseTransform):
    ...
    def forward(
        ...
        points,
        radar, 
        sensor2ego, # cam 2 lidar
        lidar2ego, # None
        lidar2camera, # None
        lidar2image, # lidar 2 img
        cam_intrinsic, # intrinsic
        camera2lidar, # cam 2 lidar
        img_aug_matrix, # None
        ...
        **kwargs,
        ...
        intrins = cam_intrinsic[..., :3, :3] # [4, 1, 3, 3]
        post_rots = img_aug_matrix[..., :3, :3]
        post_trans = img_aug_matrix[..., :3, 3]
        # lidar2ego_rots = lidar2ego[..., :3, :3]
        # lidar2ego_trans = lidar2ego[..., :3, 3]
        ...
        if radar is not None:
            for i in range(len(radar)):
                radar_pts = radar[i]  # shape: (n, 4)
                # pad (n, 4) -> (n, 9) by adding 5 zero columns
                pad_size = points[i].shape[1] - radar_pts.shape[1]
                radar_pts_padded = torch.nn.functional.pad(radar_pts, (0, pad_size))  # pad last dim
                # concat with points[i] (assumed shape [m, 9])
                points[i] = torch.cat([points[i], radar_pts_padded], dim=0)
        ...
        if self.add_depth_features:
            depth_in_channels += points[0].shape[1] # 1 + 9

        depth = torch.zeros(batch_size, img.shape[1], depth_in_channels, *self.image_size, device=points[0].device) # [B, N, 1, H, W]


        for b in range(batch_size):
            cur_coords = points[b][:, :3] # 좌표만 추출
            ...
            # lidar2image # NOTE 라이다 pts들을 모두 이미지 좌표게로 변환
            ...
            cur_coords = cur_coords[..., [1, 0]] # x, y 순서 바꾸기
            ...
            for c in range(on_img.shape[0]):
                masked_coords = cur_coords[c, on_img[c]].long() # pixel 좌표로 변환하기 위해 long사용 
                masked_dist = dist[c, on_img[c]]

                if self.depth_input == 'scalar':
                    depth[b, c, 0, masked_coords[:, 0], masked_coords[:, 1]] = masked_dist # [B, N, C, H, W]
        ...
        self._sparse_depth_map = depth  # store for reuse by depth supervision
        x = self.get_cam_feats(img, depth) # img_metas
        # x = self.get_cam_feats(img, depth,img_metas) # 이거 사용 안 함
        use_depth = False
        if type(x) == tuple:
            x, depth = x 
            use_depth = True
