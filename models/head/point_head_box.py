...
class PointHeadPreMask(PointHeadTemplate):
    """
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['point_cls_scores'] = torch.sigmoid(point_cls_preds_max) # 각 포인트에 대해 점수 할당
        
        ret_dict = {'point_cls_preds': point_cls_preds}
        if True or self.training: # 이거 실행
            targets_dict = self.assign_targets(batch_dict)
            ret_dict['point_cls_labels'] = targets_dict['point_cls_labels']
            batch_dict['point_cls_labels'] = targets_dict['point_cls_labels'] # -1은 학습에서 무시, 0은 background, 1은 foreground
