...
class KRadarDetection_v2_3(Dataset):
    """
    ...
    def load_dict_item(self, path_data, split):
        ...
        for path_header in path_data.list_dir_kradar:
            ...
            for seq in list_seqs:
                if seq.find("sparse")==-1 and ((self.portion is None) or (seq in self.portion)):
    ...
    def get_dict_cam_calib_from_yml(self):
        ...
        dir_cam_calib = osp.join(osp.dirname(self.cam_calib.dir), 'calib_seq_v2')
        ...
        for seq_dir in list_seq_dirs:
            ...
            dict_cam_calib[seq_key] = get_matrices_from_dict_calib(dict_temp) # img_size, intrinsics, distortion, T_ldr2cam
    ...
    def get_camera_param(self, dict_item):
        ...
        if seq_cam_calib is None:
            print('비상비상, camera parameters가 없다')
            seq_cam_calib = next(iter(self.dict_cam_calib.values()))
    ...
    def __getitem__(self, idx):
        ...
        if self.depth_gt_enabled and self.depth_gt_dir is not None:
            ...
            if osp.exists(path_depth):
                ...
                dict_item['depth_gt'] = np.zeros((1, self.scale_size[0]//8, self.scale_size[1]//8), dtype=np.float32)
...
if __name__ == '__main__':
    kradar_detection = KRadarDetection_v2_3(split='all')
