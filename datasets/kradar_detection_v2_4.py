...
* date: 2026-01-20
* update log:
    * v2.4: BEVDepth format support
    * applies sequence-specific camera intrinsics
    * BEVDepth mats_dict format output
    * sweep_imgs format output
    * No scaling (crop only)
======== ------------------------------ ===========
'''
...
class KRadarDetection_v2_4(Dataset):
    """
        Dataset for camera-only BEVDepth training on K-Radar
        Outputs BEVDepth-compatible format (sweep_imgs, mats_dict, lidar_depth)
    """
    def __init__(self, cfg=None, split='all'):
        ...
        ### --------------- Camera Branch --------------- ###
        self.depth_gt_cfg = self.cfg.get('depth_gt', None)
        if self.depth_gt_cfg is None:
            self.depth_gt_enabled = False
            self.depth_gt_dir = None
        else:
            self.depth_gt_enabled = self.depth_gt_cfg.get('enabled', False)
            self.depth_gt_dir = self.depth_gt_cfg.get('dir', None)
    ...
    def get_dict_cam_calib_from_yml(self):
        dict_cam_calib = dict()
        self.map_x_by_seq = dict()
        self.map_y_by_seq = dict()
        
        dir_cam_calib = osp.join(self.cam_calib.dir, 'calib_seq_v2') # resources/cam_calib/calib_seq_v2
        list_seq_dirs = sorted(os.listdir(dir_cam_calib))

        if self.cam_preprocess is not None:
            # crop 길이 계산
            crop_h, crop_w = self.crop_size # [640, 1280]
            orig_h = self.ori_size[0]  # 720
            self.y_crop = orig_h - crop_h  # 80

        for seq_dir in list_seq_dirs:
            if not seq_dir.startswith('seq_'):
                continue
            seq_str = seq_dir.split('_')[-1]
            try:
                seq_key = str(int(seq_str))
            except ValueError:
                continue
            yml_path = osp.join(dir_cam_calib, seq_dir, 'cam_1.yml')
            if not osp.exists(yml_path):
                continue
            with open(yml_path, 'r') as yml_file:
                dict_temp = yaml.safe_load(yml_file)
            dict_cam_calib[seq_key] = get_matrices_from_dict_calib(dict_temp) # obtains: (img_size, intrinsics, distortion, T_ldr2cam)

            if self.cam_calib is not None:
                img_size, intrinsics, distortion, T_ldr2cam = dict_cam_calib[seq_key]

                # intrinsic 보정
                ncm, _ = cv2.getOptimalNewCameraMatrix(intrinsics, distortion, img_size, alpha=0.0)
                intrinsics[:, :] = ncm
                self.map_x_by_seq[seq_key], self.map_y_by_seq[seq_key] = cv2.initUndistortRectifyMap(
                    intrinsics, distortion, None, ncm, img_size, cv2.CV_32FC1
                ) # 나중에 get_camera에서 사용할 거 미리 생성하기

                # extrinsic 보정
                rotation = T_ldr2cam[:, :3] # (3, 3)
                calib_vals = -1 * np.array([-2.54,0.3,0.7]).reshape(3,) # [2.54, -0.3, -0.7]
                T_ldr2cam[:, 3] = T_ldr2cam[:, 3] + rotation @ calib_vals

                # update
                dict_cam_calib[seq_key] = (img_size, intrinsics, distortion, T_ldr2cam)
            ...
            if self.cam_preprocess is not None:
                ...
                # update
                dict_cam_calib[seq_key] = (img_size, intrinsics, distortion, T_ldr2cam) # update
        
        # if self.map_x_by_seq:
        #     first_seq_key = next(iter(self.map_x_by_seq.keys()))
        #     self.map_x = self.map_x_by_seq[first_seq_key]
        #     self.map_y = self.map_y_by_seq[first_seq_key]
    ...
    def get_camera_img(self, dict_item):
        ...
        dict_item['front0'] = img_front[:,:1280,:] # get left image
        
        # 이미지 보정하기
        if self.cam_calib is not None:
            img_front = dict_item['front0']
            seq = dict_item['meta']['seq']
            # map_x = self.map_x_by_seq.get(seq, self.map_x)
            # map_y = self.map_y_by_seq.get(seq, self.map_y)
            map_x = self.map_x_by_seq[seq]
            map_y = self.map_y_by_seq[seq]
            img_undistorted = cv2.remap(img_front, map_x, map_y, cv2.INTER_LINEAR)
        ...
        if self.cam_preprocess is not None:
            ...
            if scale_h != crop_h or scale_w != crop_w:
                img_processed = cv2.resize(img_cropped, (scale_w, scale_h), interpolation=cv2.INTER_AREA)
            else:
                img_processed = img_cropped  # No resize needed
                
            # 이미지 정규화
            img_processed = img_processed.astype(np.float32) # uint8 -> float32
            mean = np.array(self.mean, dtype=np.float32)
            std = np.array(self.std, dtype=np.float32)
            img_normalized = (img_processed - mean) / std
    ...
    def get_camera_param(self, dict_item):
        ...
        seq = dict_item['meta']['seq']
        seq_cam_calib = self.dict_cam_calib.get(seq)
        intrinsics = seq_cam_calib[1] # [3, 3]
        intrinsics = np.insert(np.insert(intrinsics, 3, [0,0,0], axis=1), 3, [0,0,0,1], axis=0) # [3, 3] -> [4, 4]
        T_ldr2cam = seq_cam_calib[3] # [3, 4]
    ...
    def get_bevdepth_mats(self, dict_item):
        """
        BEVDepth mats_dict 형식으로 변환하는 함수

        Returns mats_dict with:
            - sensor2ego_mats: [1, 1, 4, 4] - camera to ego transform
            - intrin_mats: [1, 1, 4, 4] - camera intrinsic
            - ida_mats: [1, 1, 4, 4] - Image Data Augmentation matrix
            - bda_mat: [4, 4] - BEV Data Augmentation matrix
        """
        cam_param = dict_item['cam_param']

        # sensor2ego: camera to ego (in K-Radar, ego = lidar)
        # T_cam2ldr = camera2lidar
        sensor2ego = cam_param['camera2lidar']  # [1, 4, 4]
        sensor2ego = np.expand_dims(sensor2ego, axis=0)  # [1, 1, 4, 4]

        # intrinsic matrix
        intrin = cam_param['camera2image']  # [1, 4, 4]
        intrin = np.expand_dims(intrin, axis=0)  # [1, 1, 4, 4]

        # IDA matrix (Image Data Augmentation)
        # For crop-only preprocessing, ida_mat encodes the crop transformation
        # crop shifts the principal point: cy' = cy - y_crop
        # Since we already applied this in get_dict_cam_calib_from_yml,
        # ida_mat is identity for the model (crop is already in intrinsic)
        ida_mat = np.eye(4, dtype=np.float32)
        ida_mat = np.expand_dims(np.expand_dims(ida_mat, axis=0), axis=0)  # [1, 1, 4, 4]

        # BDA matrix (BEV Data Augmentation) - no augmentation
        bda_mat = np.eye(4, dtype=np.float32)  # [4, 4]

        dict_item['mats_dict'] = {
            'sensor2ego_mats': sensor2ego.astype(np.float32),
            'intrin_mats': intrin.astype(np.float32),
            'ida_mats': ida_mat.astype(np.float32),
            'bda_mat': bda_mat.astype(np.float32),
        }
    ...
    def get_ldr64(self, dict_item):
        if self.ldr64.processed: # with attr & calib & roi
            ...
            if self.ldr64.inside_ldr64: # clean points very close to LiDAR
            ...
            if self.ldr64.calib:
                ...
                pc_lidar[:,:3] = pc_lidar[:,:3] + calib_vals # NOTE [-2.54, 0.3, 0.7] is added to convert to radar coodinate
    ...
    def __getitem__(self, idx):
        ...
        # BEVDepth format
        dict_item = self.get_bevdepth_mats(dict_item) if self.item['cam'] else dict_item
        ### Camera ###

        # Depth GT loading
        if self.depth_gt_enabled and self.depth_gt_dir is not None:
            seq = dict_item['meta']['seq']
            camf = dict_item['meta']['idx']['camf']
            path_depth = osp.join(self.depth_gt_dir, f"{seq}", f"cam-front_{camf}.npy")
            if osp.exists(path_depth):
                depth_gt = np.load(path_depth)
                # depth_gt shape: [H, W] or [1, H, W]
                if depth_gt.ndim == 2:
                    depth_gt = depth_gt[None, ...]  # [1, H, W]
                dict_item['depth_gt'] = depth_gt.astype(np.float32)
            else:
                # Fallback: create zero depth map with scale_size
                dict_item['depth_gt'] = np.zeros((1, self.scale_size[0], self.scale_size[1]), dtype=np.float32)
                # print('Error! depth gt does not exist!')
        return dict_item
    ...
    def collate_fn(self, list_batch):
        ...
        for k in list_keys:
            if k not in ['cam_param', 'mats_dict']:
                dict_batch[k] = []
        ...
        for batch_idx, dict_item in enumerate(list_batch):
            for k, v in dict_item.items():
                if k == 'meta':
                ...
                elif k in ['rdr_sparse', 'ldr64', 'front0', 'ldr64_uem', 'depth_gt']:
                    ...
                    # BEVDepth mats_dict
                    if 'mats_dict' not in dict_batch:
                        dict_batch['mats_dict'] = {key: [] for key in v.keys()}
                    for mat_name, mat_value in v.items():
                        if isinstance(mat_value, np.ndarray):
                            mat_value = torch.from_numpy(mat_value)
                        dict_batch['mats_dict'][mat_name].append(mat_value.float())

        dict_batch['batch_size'] = batch_idx+1
        
        ### ----- LSS ----- ###
        if 'ldr64' in dict_batch:
        ...
        ### ----- BEVDepth mats_dict ----- ###
        if 'mats_dict' in dict_batch:
            for mat_name in dict_batch['mats_dict']:
                dict_batch['mats_dict'][mat_name] = torch.stack(dict_batch['mats_dict'][mat_name], dim=0)
        ### ----- BEVDepth mats_dict ----- ###

        return dict_batch

if __name__ == '__main__':
    kradar_detection = KRadarDetection_v2_4(split='all')
