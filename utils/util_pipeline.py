...
def build_network(p_pline):
    mode = getattr(p_pline, 'mode', 'train')  # default 'train' for backward compatibility
    return build_skeleton(p_pline.cfg, mode=mode)
...
def build_optimizer_finetune(p_pline, model):
    """
    * created: hj park
    * usage: optimizer builder before finetuning
    """
    lr = p_pline.cfg.OPTIMIZER.LR
    betas = p_pline.cfg.OPTIMIZER.BETAS
    weight_decay = p_pline.cfg.OPTIMIZER.WEIGHT_DECAY
    ...
    # requires_grad=True인 파라미터만 수집
    trainable_params = [param for param in model.parameters() if param.requires_grad]
    

    if p_pline.cfg.OPTIMIZER.NAME == 'Adam':
        optimizer = torch.optim.Adam(trainable_params, lr=lr, betas=betas, weight_decay=weight_decay)
    elif p_pline.cfg.OPTIMIZER.NAME == 'AdamW':
        optimizer = torch.optim.AdamW(trainable_params, lr=lr, betas=betas, weight_decay=weight_decay)
    elif p_pline.cfg.OPTIMIZER.NAME == 'SGD':
...
def build_scheduler(p_pline, optimizer):
    ...
    elif p_pline.cfg.OPTIMIZER.SCHEDULER == 'OneCycleLR':
        max_lr = p_pline.cfg.OPTIMIZER.get('MAX_LR', p_pline.cfg.OPTIMIZER.LR * 3)
        return torch.optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=max_lr, total_steps=total_iter,
