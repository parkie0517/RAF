"""
* combine results for multiclass results
"""
import os
import re
import pandas as pd
...
def parse_and_format_final(folder_path):
        ...
        if 'complete_results.txt' in files:
            ...
            try:
                ...
                for block in blocks:
                    ...
                    cls_match = re.search(r'cls:\s*(.*)', lines[1])
                    cls_name = cls_match.group(1).strip() if cls_match else 'unknown'
                    ...
                    long_format_results.append({
                        'epoch': epoch, 'conf_threshold': conf_threshold, 'cls': cls_name,
    ...
    df_melted = df_long.melt(
        id_vars=['epoch', 'conf_threshold', 'cls', 'condition'],
    ...
    # condition_order = ['all', 'close', 'mid', 'far']
    ...
    # 클래스 정렬 순서(예: sedan -> bus)를 지정합니다.
    preferred_class_order = ['sed', 'sedan', 'bus']
    unique_classes = list(df_melted['cls'].unique())
    class_order = [cls for cls in preferred_class_order if cls in unique_classes]
    class_order += [cls for cls in unique_classes if cls not in class_order]
    df_melted['cls'] = pd.Categorical(df_melted['cls'], categories=class_order, ordered=True)
    
    # 3. 풀어진 데이터를 다시 피벗하여 최종 형태로 만듭니다.
    df_final = df_melted.pivot_table(
        index=['epoch', 'conf_threshold', 'cls', 'metric'],
        columns='condition',
        values='value'
    ).reset_index()
    ...
    # 에포크 내에서 클래스 순서가 먼저, 이후 confidence 순서로 정렬되도록 설정합니다.
    df_final = df_final.sort_values(by=['epoch', 'cls', 'conf_threshold', 'metric']).reset_index(drop=True)
    
    return df_final


if __name__ == '__main__':
    model_name = 'CLEAR' # logs 폴더 안에 이름
    tag = 'test_BEVDepth'
